		...
		progressDiv.innerHTML = `
			<div style="text-align: center;">
				...
				<div style="position: relative; width: 100%; height: 16px; margin-top: 8px; background-color: #e5e7eb; border-radius: 9999px; overflow: hidden;">
					<div style="position: absolute; top: 0; left: 0; height: 100%; background-color: #4ade80; width: ${
						(progressData.totalStars / 35) * 100
					}%;"></div>
				</div>
		...
		<Fragment>
			...
			<AlertModal
				...
				<motion.div
					...
					<div
						className="w-44 h-44 relative rounded-full overflow-hidden border-4"
						style={{ borderColor: "#22c55e" }}>
						<img
							src={currentProfile.url}
							alt={currentProfile.alt}
							className="w-auto h-48 absolute -top-2 object-cover"
						/>
					</div>
					...
					<div className="text-center">
						...
						<p className="mt-2 italic mb-4" style={{ color: "#4b5563" }}>
							"{randomQuote}"
						</p>
					</div>
					...
					{showProgress && (
						<div className="w-full space-y-4">
							<div className="text-center">
								...
								<div className="relative w-full h-2 mt-2 bg-gray-200 rounded-full overflow-hidden">
									<div
										className="absolute top-0 left-0 h-full bg-green-400 transition-all"
										style={{
											width: `${(progressData.totalStars / 35) * 100}%`,
										}}></div>
								</div>
					...
					<Button className="w-full" variant="success" onClick={handleDownload}>
