...
function QuizOption({ quiz, filler }: Props) {
	...
	const questions = JSON.parse(
		localStorage.getItem("__questions__") || "{}",
	) as undefined | Record<FillerProp, string>;
		...
		<button
			className={twMerge(
				answer === text
					...
					: "bg-slate-500/30 text-white hover:bg-green-600/50",
				"rounded-md text-start p-3 cursor-pointer",
			)}
