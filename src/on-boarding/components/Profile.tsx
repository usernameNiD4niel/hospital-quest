...
export default function Profile() {
	...
	const getVisibleProfiles = () => {
		...
		const prof = [profiles[prev], profiles[currentIndex], profiles[next]];
		return prof;
	};
	...
	return (
		<div
			{...swipeHandlers}
			className="relative flex items-center justify-center overflow-hidden h-60 z-0">
			<input
				type="hidden"
				name="currentIndex"
			...
			{visibleProfiles.map((profile, index) => (
				<motion.img
					...
					alt={profile.alt}
					onClick={() =>
						handleImageClick((currentIndex - 1 + index) % profiles.length)
					}
